...
base_domain（从 `router/config.ini.example` 复制）。这几个键的值里**不要写行内注释**——
configparser 会把注释并进值，synth 一律**拒**而不替你剥（注释另起一行）。`account_id`
只在 synth 时读一次并当场校验（非空、正好 12 位数字），前端桶名、edge role 的埋点
DynamoDB 资源 ARN 与栈 Environment 三处共用那个值：空账号在桶名那条响亮，在埋点 ARN 上
却是**静默**的（PutItem 全被拒，而埋点异常一律吞掉 ⇒ 那个副本区静默零数据）。**`frontend_bucket` 那一行不要动**：
桶名是**约定**而不是可配置项——它固定为 `site-frontend-{account_id}`，键保留模板形态，
栈 synth 时按 `[AWS] account_id` 插值（与 `site-builder/config.ini [Deployer] frontend_bucket`
同一约定，所以两份文件里都不用手填账号）。**填别的值 synth 会直接拒**：资产里有四个生产方
把这个名字写死了（`deployer/infra/app.py` 的 IAM 资源 ARN 与 `FRONTEND_BUCKET` 环境变量、
`panel/deploy_panel.py` 的前端上传、经那个环境变量取值的 upload/undeploy/mark_job、
`scripts/verify_deployed_components.py` 的核对），只改这一个键的结果是 Edge 去读一个没人
