...
#
# **不要给"裸的内部资源名"加规则**（`某workload-acceptance-patrol-*` 这类角色名前缀）。
# 2026-08-27 有过一次这样的泄漏——写在 commit message 里、被人读出来的，不是扫出来的——
# 当时提议过补规则。**量测之后否掉了**，基准是那次的 38 提交待推区间
# （diff 新增行 + 全部 commit message，共 14202 行）：
#
#   - `[a-z][a-z0-9]+(-[a-z0-9]+){2,}`（3 段以上小写 kebab，是能匹配上那个名字的最窄规则）
#     命中 **1969 行 ≈ 全部行的 14%**。而且它命中最多的东西是信任边界基线**自己的指纹**
#     （`a0-6c58-cde7-89bd` 这种）——那套"十六进制每 4 位分组"的写法**正是为了避开本脚本
#     的 `[0-9]{12}` 假阳性**才存在的，加上这条规则它自己就变成大规模假阳性。
#   - `` `[a-z][a-z0-9-]+-\*` ``（反引号包裹、以 `-*` 结尾，即那次泄漏的实际书写形态）
#     命中 7 行 / 5 个不同值，**全部合法**（`site-rt-*` / `invoke-*` / `replace-*` /
#     `site-*` / `cdk-hnb659fds-assets-*`），真阳性 **0**；而且它连裸的非通配形态
#     （`某workload-monitoring-cloudtrail-to-logs`）都抓不住。
#
# 结论：**这一类没有可供正则锚定的语法特征**，而宽到能抓住它的规则会让整个脚本变弱——
# 反复的假阳性会训练出无脑 `--allow-hits`，那时真命中也跟着放行（本脚本存在的理由就是
# 消灭这个）。本地放 denylist 也不成立：要有效就得把那些名字写进去，那它不能被跟踪
# （否则等于重造泄漏），不跟踪又既不随仓库分发、也覆盖不了还没人想到的名字。
# **这一类靠人判断，而扫描器结构上帮不上忙**——说清这一点很重要：本脚本只做
# `文本 → grep PATTERN`，**没命中 PATTERN 的文本一个字也不会显示给人**，最后只打印
# 一行 clean。所以"扫过了"绝不等于"有人看过那些文本"。补上 commit message 那个来源，
# 修掉的是"这段文本连被 PATTERN 过一遍的机会都没有"，**不是**"人现在能看到它了"。
# 推公开 remote 前，裸内部资源名这一类必须有一个**独立的人工审阅步骤**，与本脚本并列：
#
#     git log --format=fuller origin/master..HEAD    # 逐条读 message，找内部命名/主机名/端点
#
# 这一步不能被本脚本的 clean 代替。
# 想重新提议时先复跑一遍量测（换成当时的区间）：
#   { git diff <range> | awk '/^\+\+\+ /{next} /^\+/{print substr($0,2)}'; \
#     git log --format='%B' <A..B>; } | grep -cE '<候选规则>'
