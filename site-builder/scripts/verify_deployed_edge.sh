...
#
# 为什么必须查产物而不是本地源码：Lambda@Edge 既不支持环境变量也没有层，所以配置
# （表名、**会话验签公钥**、开关）由 CDK 在 synth 时**字符串替换**注入，锁定的
# cryptography 闭包被**交叉安装**进 asset 目录。于是有四类事故本地完全看不出来：
#   ① synth 期取公钥失败（KMS 被拒 / 限流 / synth 解释器缺 boto3+cryptography）→ `stack.py`
#      走离线降级：注入占位符、放哨兵文件、**不** vendoring → 线上全员验签失败或冷启动
#      import 失败（3c-final 之前这一类是"SSM 读取失败"，形态换了、后果没换）；
#   ② 注入的公钥不是 config 里那把（换 key 没重部 / 陈旧 cdk.out）→ kid 集合照样对，
#      而该 kid 签的每一个会话都 302；
#   ③ 用了陈旧 cdk.out → 部署出去的是上一版代码，而 git 显示已修复；
#   ④ CloudFront 仍指向旧 Lambda 版本 → 新版本存在但没有流量走它。
# 「测试全绿 + git 已提交」对这四类一点保护都没有——只有下载产物逐行比对 + 逐把公钥对账才行。
#
# 用法：
