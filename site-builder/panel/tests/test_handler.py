...
# 这个代价，前提**只有一条**——错误文案直接给出修法。而这两种坏法的修法相反：
#   · **会被部署初始化的字段**缺失 ⇒ 绝大多数是"还没成功部署过"，修法是**部署一次**；
#   · 类型不对 ⇒ 那一行存着个用不了的值，修法是人工改库。
# （`effective_policy` 现在有**三**支：缺 `owner` 这类部署不会初始化的字段是第三支，
#  修法是人工补。这里只走 HTTP 验前两支——第三支的文案由
#  `deployer/tests/test_permissions.py` 直接盯，不必在每一层各来一遍。）
