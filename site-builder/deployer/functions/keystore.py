...
class Verdict(tuple):
    ...
    **它是普通 tuple 子类，不是 `NamedTuple`**（brief 写的是 NamedTuple）：属性
    访问、下标、解包与相等都与 NamedTuple 一致，但**没有**关键字构造、
    `_replace()` 与 `_asdict()`。没有任何技术约束要求这个形态（模块级缓存那条
    AST 守卫不看类体），2026-08-12 的独立审查把它记为一次无意的偏离——**此时
    不改**：Task 5-9 的调用点已全部按位置构造绑定到它，换基类是一次纯风险重构，
    收益只是三个本模块用不到的方法。真要改就连调用点一起改，别只换基类。
    """
    __slots__ = ()
