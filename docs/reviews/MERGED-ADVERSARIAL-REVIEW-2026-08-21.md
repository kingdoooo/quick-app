...
| 3c | **M09 的第 3 步：真修复 ②「改非对称签名」** | KMS 非对称密钥 + Edge 只放公钥 ⇒ 读 Edge 代码 / 历史 asset / SSM 都不再等于能签会话。独立设计包：Lambda@Edge 里没有 crypto 库、登录路径多一次 `kms:Sign` 往返、密钥轮转与「站点会话 / console 会话」两类受众拆分都要重做。**状态：3c-0 裁决完成（2026-09-02）→ 3c-1A 已部署（2026-09-02）→ 3c-1B 已部署（2026-09-05）→ **✅ 3c-final 已部署 2026-09-09**（asset-v1 工单 08；2A/2B/3 合一，§11.9；验证环境硬切换，窗口约 14 分钟；三把 HS SSM 参数同日删除）。** 1B 交付了 signer 切 family `kid`、L3 关闭 legacy 入口（三处 verifier 都不再接受它）、一次真实 v1→v2 轮转 + 生产回滚演示、v1 两把 SSM 参数已删（不可逆）；**④ 观察窗口经操作者裁决跳过**（单人账号 + 只读读数已证实归零，且没做"缩短窗口版"），因此该机制的真机证明只有 ⑨ 一次。**注意 1B 不改变本行的结论**：密钥仍是**对称** HS256，读到即可签，**A 组的 principal 计数不因 1B 变化**（61 / 可读密钥 56，1B 三轮闸门里都没动）——但 A 组里**两个产物计数确实动了**（legacy 代码目标 11→10、asset 并集 10→13，口径与逐步实测见 `docs/security/account-trust-boundary.md`）。留给 3c-3 的精确清单在 spec §6.2。**2A/2B/3 已作为 3c-final 一并实施（工单 08，2026-09-09；切换后 schema 6 基线实测「可签会话」= 15，与下面量测的 15 一致）**（术语见根 `CONTEXT.md`，ADR 0001 到 0003、0005）：见 `docs/superpowers/specs/2026-08-28-asymmetric-session-signing-spec.md`。**量测过的收益边界（tracked 证据：`docs/security/3c-impersonation-surface.json`，可用 `scripts/probe_impersonation_surface.py` 复跑）**：今天 56 能签 → 3c 后**冒充面 19（这是已知下界，不是上界）** = 能签会话 15 ∪ 能替换运行中 Edge 验签代码 17。**分包**：3c-0 spec+spike → 3c-1A verifier 认新形态 → 3c-1B HS signer 切 `kid`+轮转演练 → **3c-2A 闸门与验收先认 KMS（独立发布单元）** → 3c-2B signer 切 `kms:Sign` → 3c-3 退役清理。**闸门与验收必须前移到它们要观察的那次变化之前**，堆到清理包 = 整个迁移期间 false-green。<br>**两条已被推翻、别再引用的旧说法**：① ~~"残留那 12 个全都持 `replace-platform-code` ⇒ 限制性 key policy 不缩小集合"~~——那条推导的依据是个坏代理（见 3g）；实测边际收益是 **−1 个 principal**，而且限制性 key policy **结构上收不掉"劫持 signer"**那条路（恶意代码是以 signer 角色身份调 KMS 的）。结论仍是不值得，但理由换了。② ~~"A 62→41 / 密钥 57→36"~~ 那对数字来自更早的口径，未经本轮重测，不要引用。**2026-09-06 裁定**：2A、2B、3 **合并为 3c-final**，验证环境**硬切换**（现存会话作废），HS256 与 legacy 入口不进资产；理由与全部条款见 spec §11.9、ADR 0005。**✅ 3c-final 已部署 <日期>**（asset-v1 工单 08）：两把 KMS 非对称 CMK、signer 切 `kms:Sign`、三处 verifier 只认 RS256、HS 密钥材料与 legacy 入口删净、闸门加 KMS 分节、验收登录态改夹具签发器 |
...
| 6 | ~~M07（含闸门缺口）~~（**✅ 2026-09-07 已修并部署**，asset-v1 工单 04：三个平台 Function URL 的 resource policy 由共享的 `deployer/functions/function_url_policy.converge` 每次按期望集合等值收敛——读回、替换同名内容不对的、补缺、删野 Sid、写后读回核对；Sid 统一 `edge-invoke` / `edge-invoke-function`；`verify_deployed_components` ④ 对三条都断言）、~~M08~~ → **删脚本**（新账号没有存量站点要迁 blue/green，2026-09-06）**✅ 已删（工单 08）**、M10、~~M12~~（3c-1B ticket 19 已修） | 部署期正确性；都是小改 |
