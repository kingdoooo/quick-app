...
| 3b | ~~**M09 的第 3 步：真修复 ①「收窄 CodeBuild 对 bootstrap 桶的读权限」**~~ **✅ 2026-08-27 已部署** | **做法**：那条整桶读是 CDK 给 `BuildSpec.from_asset()` 自动加的（`Asset.grantRead()` 授的是**桶**不是对象），唯一用途是取项目自己那 25 行 buildspec ⇒ 用自定义 `cb.BuildSpec` 子类把原文**逐字节内联**进模板，权限**整条消失**（不是收窄）。**实测落地数字**：A 62→61、可读密钥 57→56、`platform-overbroad` 1→0；带活密钥的 asset 仍 9、Edge 代码目标仍 10、coverage 774→774、B 的 22/43 一字未动。**验收**：部署后静态三项全绿（`NO_SOURCE` + buildspec 逐字节相同 + S3 权限全集精确等于两条）、完整 E2E 10 passed（37m21s）、当天 4 次 `site-package` 构建全部 SUCCEEDED 且都用内联 buildspec。设计与计划见 `docs/superpowers/specs/2026-08-27-codebuild-bootstrap-read-narrowing-spec.md` + 同名 plan。**`--ignore-scripts` 仍然必须留着**（构建容器里任意代码执行仍能读 `validated/*`、写 `artifacts/*`）。**顺带纠正两处流传的说法**：① 这条**不会**移除约 21 个 principal——那 21 个要等 asset 里不再有活密钥，即 3c；② 「唯一的隔断是 `--ignore-scripts`」不准确——站点**自己的** package.json 与 `backend/.npmrc` 由合同校验器在 CodeBuild 之前就拒（两道），但**依赖里**的生命周期脚本只有那条 flag（`_scan_package_json` 不看 `dependencies`，且 `.tgz` 不在扫描后缀里，已用生产校验器本体实测）。**下一条是 3c** |
