...
| 3h | **`/login` 的默认 redirect 指向一个不存在的 apex**（新，2026-09-06 生产实测；与 3c 无关，独立小修） | `auth/login_handler.py` 的 `redirect = qs.get("redirect", f"https://{base}/")` 缺省指向 **apex**，而平台分发的 alias 只有 `*.{base}`、Route53 只有那条通配记录——**通配既不匹配 apex，CloudFront 的通配 alias 也不匹配**。症状：直接打开 `/login` → 授权成功 → `/callback` **正常 302 并种下 `sb_session`** → 浏览器落在 apex 上显示 `ERR_CONNECTION_CLOSED`，**登录其实成功了但看起来像失败**（auth 日志 0 错误，排查方向会被带偏；2026-09-06 先查了 CloudFront alias 与 Route53 才定位）。`_is_safe_redirect` 放行 apex 是对的（属 BASE_DOMAIN），**这不是校验缺口**，只是缺省值指向了本部署里不存在的东西。**建议改默认到 `https://console.{base}/`**（最小改动、不碰基础设施与证书；另两条路是给 apex 加记录+SAN，或无 redirect 时返回说明页）。注意 Edge 的未登录 302 **带**原始 `?redirect=`，不受影响 |
