...
> **2026-09-06 起本表按"交付物是可分发资产"重读**（ADR 0005、spec §11.9）：验证环境的迁移
> 步骤不再是工作项；每条的必要性看"对在自己账号部署这套资产的采用者是否成立"。受影响的行：
> 3c（2A/2B/3 合为 3c-final）、3d（降为部署建议）、6（M08 改删）；新增 11–15 行。**行号只是编号，不是顺序**：
> 第 11 行起的先后由 spec §11.9 第 12 条与 `.scratch/asset-v1/` 的工单给；第 6、9、10 行里与采用者无关的项不阻塞 v1。
...
| 1 | ~~**M01**~~ **✅ S1 加固包已部署** | 唯一一条真正的跨租户数据读写。改为精确 ARN 枚举（**不是** v1 说的一行正则） |
| 2 | ~~**M02**~~ **✅ S1 加固包已部署** | 抵消了已经付过成本的 Edge 加固；三个 writer 统一为「判不出就拒绝投影」 |
...
| 3b | ~~**M09 的第 3 步：真修复 ①「收窄 CodeBuild 对 bootstrap 桶的读权限」**~~ **✅ 2026-08-27 已部署** | **做法**：那条整桶读是 CDK 给 `BuildSpec.from_asset()` 自动加的（`Asset.grantRead()` 授的是**桶**不是对象），唯一用途是取项目自己那 25 行 buildspec ⇒ 用自定义 `cb.BuildSpec` 子类把原文**逐字节内联**进模板，权限**整条消失**（不是收窄）。**实测落地数字**：A 62→61、可读密钥 57→56、`platform-overbroad` 1→0；带活密钥的 asset 仍 9、Edge 代码目标仍 10、coverage 774→774、B 的 22/43 一字未动。**验收**：部署后静态三项全绿（`NO_SOURCE` + buildspec 逐字节相同 + S3 权限全集精确等于两条）、完整 E2E 10 passed（37m21s）、**当天 20 次 `site-package` 构建（13:30–14:17）全部 SUCCEEDED 且 20/20 都用内联 buildspec**（流传的「4 次」是 13:36 前的旧计数，已按生产实况纠正）。设计与计划见 `docs/superpowers/specs/2026-08-27-codebuild-bootstrap-read-narrowing-spec.md` + 同名 plan。**`--ignore-scripts` 仍然必须留着**（构建容器里任意代码执行仍能读 `validated/*`、写 `artifacts/*`）。**顺带纠正两处流传的说法**：① 这条**不会**移除约 21 个 principal——那 21 个要等 asset 里不再有活密钥，即 3c；② 「唯一的隔断是 `--ignore-scripts`」不准确——站点**自己的** package.json 与 `backend/.npmrc` 由合同校验器在 CodeBuild 之前就拒（两道），但**依赖里**的生命周期脚本只有那条 flag（`_scan_package_json` 不看 `dependencies`，且 `.tgz` 不在扫描后缀里，已用生产校验器本体实测）。**下一条是 3c** |
| 3c | **M09 的第 3 步：真修复 ②「改非对称签名」** | KMS 非对称密钥 + Edge 只放公钥 ⇒ 读 Edge 代码 / 历史 asset / SSM 都不再等于能签会话。独立设计包：Lambda@Edge 里没有 crypto 库、登录路径多一次 `kms:Sign` 往返、密钥轮转与「站点会话 / console 会话」两类受众拆分都要重做。**状态：3c-0 裁决完成（2026-09-02）→ 3c-1A 已部署（2026-09-02）→ 3c-1B 已部署（2026-09-05）→ 下一步 3c-final（2A/2B/3 合一，§11.9）。** 1B 交付了 signer 切 family `kid`、L3 关闭 legacy 入口（三处 verifier 都不再接受它）、一次真实 v1→v2 轮转 + 生产回滚演示、v1 两把 SSM 参数已删（不可逆）；**④ 观察窗口经操作者裁决跳过**（单人账号 + 只读读数已证实归零，且没做"缩短窗口版"），因此该机制的真机证明只有 ⑨ 一次。**注意 1B 不改变本行的结论**：密钥仍是**对称** HS256，读到即可签，**A 组的 principal 计数不因 1B 变化**（61 / 可读密钥 56，1B 三轮闸门里都没动）——但 A 组里**两个产物计数确实动了**（legacy 代码目标 11→10、asset 并集 10→13，口径与逐步实测见 `docs/security/account-trust-boundary.md`）。留给 3c-3 的精确清单在 spec §6.2。**3c-2A 及之后各包仍未获实施授权**（术语见根 `CONTEXT.md`，ADR 0001 到 0003）：见 `docs/superpowers/specs/2026-08-28-asymmetric-session-signing-spec.md`。**量测过的收益边界（tracked 证据：`docs/security/3c-impersonation-surface.json`，可用 `scripts/probe_impersonation_surface.py` 复跑）**：今天 56 能签 → 3c 后**冒充面 19（这是已知下界，不是上界）** = 能签会话 15 ∪ 能替换运行中 Edge 验签代码 17。**分包**：3c-0 spec+spike → 3c-1A verifier 认新形态 → 3c-1B HS signer 切 `kid`+轮转演练 → **3c-2A 闸门与验收先认 KMS（独立发布单元）** → 3c-2B signer 切 `kms:Sign` → 3c-3 退役清理。**闸门与验收必须前移到它们要观察的那次变化之前**，堆到清理包 = 整个迁移期间 false-green。<br>**两条已被推翻、别再引用的旧说法**：① ~~"残留那 12 个全都持 `replace-platform-code` ⇒ 限制性 key policy 不缩小集合"~~——那条推导的依据是个坏代理（见 3g）；实测边际收益是 **−1 个 principal**，而且限制性 key policy **结构上收不掉"劫持 signer"**那条路（恶意代码是以 signer 角色身份调 KMS 的）。结论仍是不值得，但理由换了。② ~~"A 62→41 / 密钥 57→36"~~ 那对数字来自更早的口径，未经本轮重测，不要引用。**2026-09-06 裁定**：2A、2B、3 **合并为 3c-final**，验证环境**硬切换**（现存会话作废），HS256 与 legacy 入口不进资产；理由与全部条款见 spec §11.9、ADR 0005 |
| 3d | ~~**M09 的第 3 步：真修复 ③「迁独立成员账号」**~~ → **降为 DEPLOY.md 的部署建议**（2026-09-06，ADR 0005） | 迁账号是采用者的决定，不是资产的工程项。资产必须在共享账号里也站得住（3c-final 负责），手册建议专用账号并说明理由 |
...
| 3f | ~~**给 router 栈加 CloudFormation stack policy**~~ **✅ 2026-09-07 已修并部署**（asset-v1 工单 02：Deny `Update:*` 而非工单原话的 Replace+Delete——Edge 换码是 Lambda Code 的 Modify，见 `docs/adr/0007-*.md`；每次 router 部署改为 open → deploy → apply，`verify_deployed_edge.sh` ⑤ 只读核对；**本行的 −4 未重算**：探针要先加 `cloudformation:SetStackPolicy` 等价类，只有不持有它的 principal 才真的离开冒充面）（新，2026-08-31 量测发现；**与 3c 正交，可单独做**） | 实测：router 栈**已关联** CFN service role 且**没有 stack policy** ⇒ 任何能操作该栈的 principal 都能经 `UpdateStack`（17 个）或 `CreateChangeSet`+`ExecuteChangeSet`（16 个）替换 Edge 验签代码，**而且自己不需要 `iam:PassRole`**（CFN 会继续用栈上已关联的 role）。**这是四个候选缓解措施里唯一一次能减 4 个 principal 的**（冒充面 19→15），其余三条各减 1 或 0。反过来说：**只锁 Lambda+CloudFront 那条直接链的边际收益是 0**——持有它的人全部同时持有 CFN 那条路，与 M09 第 2 步"收窄 invoke"是同一个假修复形状。证据：`docs/security/3c-impersonation-surface.json` 的 `marginal_value_if_closed` |
| 3g | **闸门的 `replace-platform-code` 是当前基线的一处低估**（新；**与 3c 是否做无关，应尽早单独修**） | 它只模拟 `lambda:UpdateFunctionCode` 一个动作，两个方向都错。**过度声称**：对 Edge 不成立（CloudFront 关联的是**编号版本**，实测如此，而该动作只改 `$LATEST`）；对**站点函数**也不成立（M7 之后 Function URL 挂 blue/green alias，alias 指向 `publish_version` 出来的编号版本）。**同时少算**：完全没看 `cloudformation:UpdateStack`（17）与 change-set（16）这两条更宽的路，也没看 `UpdateFunctionCode(Publish=True)`。**恰好对的只有 `site-auth-service` / `site-panel`**——那两个的 Function URL 无 qualifier、服务 `$LATEST`，所以单动作就是"在该执行角色下跑任意代码"。修法：按**动作等价类 × 资源等价类**重建成完整链，判据与 `scripts/probe_impersonation_surface.py` 的 `classify()` 一致（那份已有 18 条反例 + 变形测试，见 `deployer/tests/test_probe_impersonation_surface.py`） |
| 3h | ~~**`/login` 的默认 redirect 指向一个不存在的 apex**~~ **✅ 2026-09-07 已修并部署**（asset-v1 工单 01：缺省改为 `https://console.{base}/`，白名单不变；新，2026-09-06 生产实测；与 3c 无关，独立小修） | `auth/login_handler.py` 的 `redirect = qs.get("redirect", f"https://{base}/")` 缺省指向 **apex**，而平台分发的 alias 只有 `*.{base}`、Route53 只有那条通配记录——**通配既不匹配 apex，CloudFront 的通配 alias 也不匹配**。症状：直接打开 `/login` → 授权成功 → `/callback` **正常 302 并种下 `sb_session`** → 浏览器落在 apex 上显示 `ERR_CONNECTION_CLOSED`，**登录其实成功了但看起来像失败**（auth 日志 0 错误，排查方向会被带偏；2026-09-06 先查了 CloudFront alias 与 Route53 才定位）。`_is_safe_redirect` 放行 apex 是对的（属 BASE_DOMAIN），**这不是校验缺口**，只是缺省值指向了本部署里不存在的东西。**建议改默认到 `https://console.{base}/`**（最小改动、不碰基础设施与证书；另两条路是给 apex 加记录+SAN，或无 redirect 时返回说明页）。注意 Edge 的未登录 302 **带**原始 `?redirect=`，不受影响 |
| 3i | ~~**panel `/api/session-callback`：一次性升级码先烧后取密钥**~~ **✅ 2026-09-06 已修并部署**（3c-1B-G B2；`console_session.ensure_signing_material()` 由 handler 在 `consume_code` **之前**调，`console_cookie` 仍是 panel 唯一签发点；判据是"断言 `consume_code` 一次都没被调用" + 正对照；`verify_console_e2e` 72/72） | `panel/handler.py` 的 `CALLBACK` 分支先 `console_session.consume_code(...)`（DynamoDB 条件写，**不可回退地**作废该 jti），之后才在 `console_cookie()` 里取 console 签发密钥（`_signing_key()` → `ssm:GetParameter`）。取密钥失败 ⇒ 500，而那枚码已作废。失败面与 ticket 20 同源（`SESSION_KEYS_JSON` 缺失/非 JSON/family 缺 current、AccessDenied、ParameterNotFound）。**比 20 低一档**：重试只需 auth 那一跳 302（凭仍有效的 `sb_session` 换新码），不必走完整 Cognito 往返；但配置修好前每次进控制台都撞一次。**设计点**：panel 的唯一签发点是 `console_cookie`，而 signer AST 守卫禁止 `handler.py` 出现任何 mint ⇒ 要么加一个"预取签发材料"的调用，要么把 `console_cookie` 拆成取材料 + 签两步（两条各有代价，先定再写）|
| 3j | **闸门完整性收尾包（3c-1B-G）**：四道闸门能在它们本该抓住的漂移上报绿 —— **✅ 2026-09-06 已修并部署** | 整包 `/code-review` + Codex 复核**各自独立**得出同一结论：七套单测 2950 全绿而闸门判据坏了。逐条：**① family separation 的真机闸门失去隔离变量**（`_session_mint` 由 token_use 推导 family ⇒ 造不出跨 family 同形态 token，探针同时改两个变量；用真源 Edge 造过反例：把 console key 注入 site allowlist，Edge 先验签通过再因 token_use 拒 ⇒ 302 照旧 ⇒ **失守时全绿**）。修法：`mint(family=…)` 覆盖 + 两条判据各只改一个变量 + 变形证明；语义闸门的两条 M05 同样重建（原先在 allowlist 就被拒，`token_use` 比较根本没执行）。**② 排空闸门空窗口退 0**（`--require-zero` 只报非零列）⇒ 新增 `--drain-gate {previous,legacy}`，四条判据锁进脚本、拒绝把 26 打成 2.6。**③ preflight 打印 RED 仍 exit 0**、且轮转一轮后因写死 v2 而 `DuplicateSectionError`（六套件在 collection 期炸、汇总显示"红 0 条"）。**④ key material 无全局唯一性**：实测**曾接受**两个不同 family 的 kid 共用同一 `ssm_param`（跨 family 又共享密钥）与 `legacy_param` 指向 HS 行 ⇒ HS 行的 `ssm_param` 改为**必须恰好等于**前缀 + kid。**⑤ 声明 key 的 coverage churn**（两轮各 305 条 `new_undecided_items`、最终人工 `--update-baseline`，违反 spec §11.8.7）⇒ 反事实比较：剔掉被声明 key 的资源类后与基线相等才整批落绿 `migration_undecided`，**残差照红**；并让 `--update-baseline` 先渲染报告再写（原先在比较之前就 return ⇒ 接受了什么不留痕）。**⑥ `signing_key` 连 previous 的 secret 一起取** ⇒ ⑩ 若先删退役参数再重部则每次登录 500，改成只 resolve current。外加五条无部署快修（scan 分页、`sys.path` 污染——`_edge_placeholder_re` 改按路径加载；`_session_keys_on_path` **只去重、`site-builder/auth` 常驻 `sys.path[0]` 未改**，列入 3c-3 清单——、裸 assert、`mkdir(mode=)` 不作用于父目录、变形助手的锚点与 `sys.modules` 生命周期）。**推翻一条**：两轮 review 都把"吸收条件要求 principal 原本已持有会话密钥 grant"报成缺陷，**不改**——原先读不到任何密钥的 principal 现在能读到是**能力面真的变大**，一句 `--new-key` 不该抹掉，已写成用例钉住意图。commit `bdabdf9`。**Codex 复审（2026-09-06）再闭合四条**：ⓐ ⑤ 的"反事实"只对**新增**成立——退役的 key 只在基线里有，对本次观测剔类等于没剔，`--retire-key` 声明过照样整批红（用生产比较器复现）⇒ coverage 成员改成**可分解**形态 `指纹|动作类|资源类,…`、归一化对**两侧**做，新增/退役/同轮全走一条判据，快照不再存按声明算的第二份；基线 **schema 4 → 5**（哈希不可逆，经一次实测迁移重写；`--migrate-baseline-only` 对 4→5 被拒）。ⓑ `--update-baseline` 预览失败（`SystemExit`）原先只打一句照写 ⇒ 现在原样传播、不写。ⓒ preflight 的 `next_kids()` 移到备份/哨兵之前（原先它失败会留下假哨兵）。ⓓ `save_token` 对已存在的 `.scratch/*` 也收 0700；`session_verify_counts` 的 docstring 改示范 `--drain-gate`，裸 `--require-zero accepted_*` 保留为诊断旗标但 stderr 提醒它不是闸门。另按建议补了 RS key_arn/spki 重复的两条直接用例。**推翻一条**：Codex 把 `_session_keys_on_path` 的 `sys.path[0]` 常驻记为"文档比代码强"——成立，措辞已改（见前句），代码改动留 3c-3。**复审二轮**（同日）再收四条小口：迁移快照的 `schema4_fingerprints` 按指纹形态校验；`--migrate-baseline-only` 对任何旧 schema 都拒（3→5 同样会落出自相矛盾的基线，已复现）；`--dump-observed` 按 0600 落盘；只解释了部分 churn 时报告不再写"归一化相等"。文档：DEPLOY.md 三处旧结论（`--update-baseline 不做比较`、"真修复没有排期"、两处 `/tmp` 的 DUMP）与基线 `note` 改准确。**三轮**：`os.open(..., 0o600)` 对已有文件不生效且跟随 symlink（Codex 用 `atb.json → victim` 复现）⇒ dump 与 token 两处落盘改为共用 `scripts/_secure_write.py`（同目录 0600 临时文件 + `os.replace`），⑥ 步剩余两处 `/tmp` 改 `.scratch/`。三轮合一 commit `56ccb2d` |
| 4 | ~~**M05**~~ **✅ S1 加固包已部署** | 给两处 verifier（`auth/session.py` 与 Edge 内嵌那份）加必填 `expected_typ`，顺带处理 `.example` 出厂开关 |
| 5 | ~~**M06** 短期缓解~~ **✅ S1 加固包已部署**；host-only 根治**未做** | 短期缓解（逐个尝试同名 cookie）门槛低、收益大；host-only 根治列入长期 |
| 6 | ~~M07（含闸门缺口）~~（**✅ 2026-09-07 已修并部署**，asset-v1 工单 04：三个平台 Function URL 的 resource policy 由共享的 `deployer/functions/function_url_policy.converge` 每次按期望集合等值收敛——读回、替换同名内容不对的、补缺、删野 Sid、写后读回核对；Sid 统一 `edge-invoke` / `edge-invoke-function`；`verify_deployed_components` ④ 对三条都断言）、~~M08~~ → **删脚本**（新账号没有存量站点要迁 blue/green，2026-09-06）、M10、~~M12~~（3c-1B ticket 19 已修） | 部署期正确性；都是小改 |
| 7 | **M03 + M16** | 必须**一起**做：强制可重放 DDL 的前提是子目录不能绕过扫描 |
| 8 | **M04 的判别实验** | 无副作用的真机实验；出结果再决定是否进队列 |
| 9 | M11、M13、M17、M21 | 把手抄的不变量变成可执行守卫 |
| 10 | M14、M15、~~M19~~（**✅ 2026-09-07 已修并部署**，asset-v1 工单 05：`_ROUTE_CACHE` 改有界 LRU、上界 1024；**miss 仍缓存是刻意的**——不缓存会让同一个被反复打的不存在 host 变成路由表热键，随机 host 洪水本就每个一次 GetItem、只受上界约束；每标签一次 GetItem 的放大不在本票射程）、M20、M22 | 卫生、文档、可观测性 |
...
| 12 | **迁移脚手架与单账号数据移出资产**（2026-09-06 新增；ADR 0005） | legacy 状态机、schema 3/4→5 通道、HS 版 runbook、blue/green 存量迁移脚本删除；`account_trust_baseline.json` 与探针结果不再 tracked；DEPLOY.md 的时间线迁到决策记录 |
