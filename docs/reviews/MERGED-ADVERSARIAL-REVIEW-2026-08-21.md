...
| 3i | **panel `/api/session-callback`：一次性升级码先烧后取密钥**（新，2026-09-05 按源码复核；与 3c-1B ticket 20 同形状） | `panel/handler.py` 的 `CALLBACK` 分支先 `console_session.consume_code(...)`（DynamoDB 条件写，**不可回退地**作废该 jti），之后才在 `console_cookie()` 里取 console 签发密钥（`_signing_key()` → `ssm:GetParameter`）。取密钥失败 ⇒ 500，而那枚码已作废。失败面与 ticket 20 同源（`SESSION_KEYS_JSON` 缺失/非 JSON/family 缺 current、AccessDenied、ParameterNotFound）。**比 20 低一档**：重试只需 auth 那一跳 302（凭仍有效的 `sb_session` 换新码），不必走完整 Cognito 往返；但配置修好前每次进控制台都撞一次。**设计点**：panel 的唯一签发点是 `console_cookie`，而 signer AST 守卫禁止 `handler.py` 出现任何 mint ⇒ 要么加一个"预取签发材料"的调用，要么把 `console_cookie` 拆成取材料 + 签两步（两条各有代价，先定再写）|
