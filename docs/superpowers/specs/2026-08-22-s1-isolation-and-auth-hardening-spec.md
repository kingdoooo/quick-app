...
**上限取 64**（v3 更正，实施期实测；原值 8 是错的，理由见下）：每次验签是一次
SHA256 HMAC（微秒级），且单条 Cookie 头受浏览器/CloudFront 约 8KB 限制、N 天然
有界——**真正在兜底的是头长度限制，显式上限只是把意图写进代码，所以必须设在任何
可达值之外**。
...
**64 是实测选的**：验完 8KB 头里塞得下的**全部**候选（约 630 条）耗 CPU **0.407ms**，
上限 8 耗 0.052ms——也就是说 8 用 0.35ms 的最坏情况 CPU 换来把缺陷重新打开。
64 的最坏情况约 0.04ms，要耗尽它需要每个域约 32 个不同路径前缀（约 16 段的 URL），
不可行。上限值与边界用例必须**由同一个常量推导**，否则改了常量而用例仍在测旧边界。
