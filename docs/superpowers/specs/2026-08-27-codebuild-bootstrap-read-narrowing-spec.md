...
3. **信任边界闸门有一个约 10 分钟的 TOCTOU**（枚举 → 逐个模拟），而
   ...
   **实测**：写下当前基线的那次运行本身窗口内就有两条 `PutRolePolicy`（CloudTrail），
   ...
   已修：模拟后再枚举一次（T3）并逐个比对 `uid`、boundary ARN **与 boundary 文档的语句**、
   带来源的全部语句、每份贡献策略的 `DefaultVersionId`，任一类漂移即作废本轮。
   **修了两轮**：第一版摘要只放 `boundary_arn`，而 boundary 不算"被附加"⇒ 它的语句不在
   `statements` 里 ⇒ 换 boundary 默认版本、ARN 不变时摘要一个字都不变，**持久漏报**
   （复审第八轮抓的，同一个「枚举范围比声称的主语窄」）。
   **口径也收窄过一次**：它是「窗口两端一致性」复查，**不是原子性**——两点比较证明不了
   中间没变过（改了又改回来 / 枚举后新建又在复查前删除 / 单次枚举分页那 ~90 秒都不可见；
   三个盲区各有一条钉住它们的用例）。详见 `account-trust-boundary.md` 的闸门 caveat 一节。
