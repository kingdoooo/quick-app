...
- **站点列表的 PV 迷你趋势（`pv7`）**——本轮范围内（Codex 审查 2026-08-14
  ...
  **成本注意（数字按实测修正，2026-08-14）**：**站点数 × 2 次 Query**，不是 1 次
  ——日聚合表一次，**今天那个分区的明细一次**（`series()` 对今天恒走实时计算，
  因为今天还没被 rollup 封口）。31 个站点 ≈ 62 次串行 Query，控制台首页可接受。
  管理员全局视图（`?all=1`）同理，站点数长到三位数时要改——**先做的是并发而不是
  缓存**（两张表都是分区 Query，天然可并行）。届时重评，本轮按实测规模直接实现。

  **更正（2026-08-15，提交 `ed1133a`）**：上一版这里写的是「明细那次的规模跟今天的
  访问量走、不跟天数走，所以单站成本没有上界可从『7 天』推出来」——**那个描述当时成立，
  现在不成立了**。`pv7()` 给今天那次明细读加了**硬上界**：`_day_rows(..., max_rows=N)`
  用 `Limit=N+1` 让**服务端截断**（不是拉回整页再在客户端 break——行数才是成本），
  多一行就 raise，`pv7` 随即降级成「未知」（`[]`）而不是给一个偏小的数。
  即首页那一面现在**两次 Query 都有上界**：聚合表那次由 `BETWEEN` 键条件封住
  （每天最多一行），今天那次由行数上界封住。**统计页那一面刻意不带上界**
  （`series()` 要精确数字，走全量分页）——两个面的成本上界不同是有意的。
