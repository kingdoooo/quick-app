...
4. **复用模块级的那个 `dynamodb` client，不要新建。** 这条被 §0.1 的探针推翻过
   一次，理由必须留在代码注释里：
   - 初版设计要新建一个 client 并给 `connect_timeout=0.3 / read_timeout=0.5`。
     **实测证明那会让每个冷容器的首次埋点全部超时**（冷连接首次 PutItem
     实测 **719ms**，而预算合计 0.8s 且分属两个阶段），超时又被规矩 3 吞掉
     ⇒ **静默丢行**——正是本 spec §0.1 用来否掉方案 c 的那种失效形状。
   - **单独的 client = 单独的连接池**，所以它无法蹭到路由查询已经建好的连接。
   - 复用则不同：`_lookup_route` 的 GetItem 在同一次调用里、埋点之前发生，
     连接此时是暖的 ⇒ 埋点稳定付约 **229ms**，与容器冷热无关。
   - 代价（已接受）：继承该 client 的默认重试策略。埋点本身不需要重试，最坏
