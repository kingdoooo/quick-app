...
2. **显式开关（副）——已部署但要临时关。** 开关为关时 key-proxy 立即拒绝全部
   Key（怀疑泄漏时的一键关闸，不必拆组件、不必重部 AgentCore）。它是应急手段，
   **不是**主门禁；实施时必须有结构性断言盯住"所有判定点都读同一个开关"，
   否则就是上面那类手抄缺陷。

   > **⚠️ 2026-08-11 修正开关的存放位置（Codex 审查 P1-1）。**
   > 本条原文写的是 `[ApiKey] enabled = false`（配置项）。**已改为
   > `site-api-keys` 表里的一条哨兵行**（`key_hash="__switch__"`），由管理员在
   > 控制台开关，`config.ini` **不再有 `enabled` 键**。
   >
   > 为什么必须改而不是两者并存：留着配置项就有**两个真源**。真实的失效链是
   > ——运维照本条原文去 `config.ini` 设 `enabled = false`，而实现只读哨兵行，
   > 于是**Key 全部继续有效**，"泄漏时一键关闸"这个安全承诺当场击穿，且现场
   > 会以为已经关了。这比没有开关更糟。
   >
   > 选哨兵行而不是配置项的三条理由：① 配置项要改文件 + 重跑部署脚本才生效，
   > 而部署脚本"收敛配置到线上"的语义会把管理员的关闸**静默覆盖成开**；
   > ② 关闸动作必须留审计（谁在何时关的），控制台路径天然经 `ops_log`；
   > ③ key-proxy 本来就每请求读这张表，开关放同表不引入新的依赖。
   >
   > 首次部署时哨兵行建成**关**（fail-closed），部署完由管理员在控制台开。
   > 控制台不可用时的应急旁路（aws CLI 直改该行）见 DEPLOY.md「⑤c」。
   > 详见 `docs/superpowers/plans/2026-08-10-phase2-m4-api-key.md` 的决定 6。

**控制台入口同步门禁**：组件未部署时，API Key 页面必须显式 disabled 并说明原因
（同 M3 对 M4/M5 占位的做法）——不得渲染一个看起来能用的表单，让用户创建出
根本不通的 Key。

**"未部署"与"已部署但关闸"必须是两个可区分的状态**（2026-08-11 追加，Codex
审查 P1-5）：控制台的门禁只看"组件是否已部署"，**不能**看开关值。否则首次部署后
哨兵行是关的 → 页面 disabled → 管理员无处点开闸 → 部署流程自锁。因此
`/api/me` 返回的是 `{"deployed": bool, "enabled": bool}` 两个字段而非单个布尔：
`deployed` 决定 UI 可用性，`enabled` 只驱动状态提示与开关初值。
