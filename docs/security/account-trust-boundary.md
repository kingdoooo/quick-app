...
| | 数量 |
|---|---|
| 今天能读明文密钥 ⇒ 能签任意人的会话 | **56** |
| B 之后能签会话（`kms:Sign` / 自助授权 / **劫持 signer 代码**） | **15** |
| B 之后能替换 CloudFront **正在执行的** Edge 验签代码 | **17** |
| **⇒ B 之后的冒充面（并集）** | **19** |

> **这个 19 是「已知下界」，不是上界**——它是按九条已建模路径量出来的并集，聚合证据里
> 带着 `known_gaps`（KMS key policy 未纳入、IAM 自助提权归本文档 B 组、
> `UpdateFunctionConfiguration` 取上界口径等）。把"我建模的路径的并集"写成"全部路径的
> 并集"是这一轮被外部复审连续咬住两次的同一个错误。

**所以下面这句旧措辞要收窄**：不能说"只做 B 就把风险压到「账号管理员」"。残留那 19 个里
确实有管理身份，但也有若干**工作负载角色**（不是人类管理员）。准确的说法是：**B 把边界从
「账号内任何只读身份」压到「能改写平台代码或能签名的身份」**。

**顺序建议**：B 关掉的是当前 56 个 principal 里绝大多数（只读级那批），
且不依赖账号迁移；A 关掉的是剩下的那批身份。先 B 后 A。
...
> **2026-09-06（3c-1B-G Codex 复审）基线口径变化：schema 4 → 5**——只动 `coverage.undecided_items`
> 的**形态**，数字口径不变。成员从不可分解的哈希改成 `<principal 指纹>|<动作等价类>|<资源类,…>`
> （三段都不是账号值：指纹与 `principals` 分节同一种，类名来自脚本的词表；仓库红线的第二层为它
> 加了专门的形态校验）。**为什么必须改**：被声明 key 的 coverage churn 要靠"剔掉那个资源类再比"
> 归因，而退役的 key 只在**基线**里有——哈希剔不掉类，`--retire-key` 声明过照样整批红（用生产
> 比较器复现）。现在归一化对两侧做，新增/退役/同轮一条判据。迁移是一次实测（哈希不可逆）：那一跑
> 仍按旧哈希比较、然后以新形态写入；`--migrate-baseline-only` 对 4→5 被拒。
...
2. **IAM 策略变更（层 B）——纯静态文本快照，不做权限分析，不进模拟器。**
   ...
   **这是这道闸门被前五轮外部复审反复点名的根因。** 所以 B 的承诺刻意收窄成一句：
...
- **枚举与逐个模拟之间有一个约 10 分钟的窗口（TOCTOU）。** 先
  `GetAccountAuthorizationDetails` 拿全量名单（T1），再逐个 `SimulatePrincipalPolicy`
  （T2）。B 组的语句抄自 T1、A 组的判定来自 T2 ⇒ 窗口内 IAM 变过的话，**一份快照会
  同时描述两个时刻的账号**。
  **这曾被写成"硬失败，所以只是可用性问题不是正确性问题"——那句话是错的**，
  `NoSuchEntity` 只咬住"模拟时角色已经不在了"这一种 churn。真正危险的是另外三类：
  同名角色被删了重建（ARN 不变、RoleId 变 ⇒ A 模拟的是新一代、B 抄的是旧一代，
  新一代多出来的 IAM 写权限归 B 管而 B 没看见）；策略在窗口内被改（uid 都没变）；
  T1 之后新建的 principal 整轮没被模拟。
  **2026-08-27 起，模拟结束后会再枚举一次（T3）并逐个比对**：`uid`（RoleId/UserId）、
  boundary ARN **与 boundary 那份文档的语句**、带来源的全部语句、每份贡献策略的
  `DefaultVersionId`。三类里任一类出现即**作废本轮**——不出结论、不写基线
  （`enumeration_drift`）。`uid` 只在进程内用、**不进基线**：合法重建每天都发生，
  记进基线等于每天红一次而没有安全含义。
  **boundary 的文档内容是补进来的第二轮**：第一版摘要只放了 `boundary_arn`，而
  boundary **不算"被附加"** ⇒ 它的语句根本不在 `statements` 里 ⇒ boundary 那份托管策略
  换了默认版本、ARN 不变时，摘要的输入一个字都不变，**而且这个漏报是持久的**
  （不需要"改了又改回来"那种巧合）。两层都受影响：B 的 boundary 指纹是 T1 抄的；
  而 `SimulatePrincipalPolicy` 不传 `PermissionsBoundaryPolicyInputList` 时用的就是实体
  **当前**那份 boundary（该参数的语义是**覆盖**已附加的那份，AWS 文档明确）。
  **三条实测事实**（生产 CloudTrail 查得）：① 同名重建在本账号是常态——2026-08-27
  有两个固定名字的角色各被重建 4 次、拿到 4 个不同 RoleId；② **写下当前这份基线的
  那次运行本身就不是原子的**——枚举 14:38:44–14:40:12、产物 14:47:05，窗口内有两条
  `PutRolePolicy`。事后核对：被改的两个角色既不在 A 也不在 B、也不是任何 boundary，
  改动只动了 `logs:`/`s3:PutObject` 的资源后缀 ⇒ **值没错，但"这是一个时刻的快照"
  当时是假的**，闸门当时无从知道；③ 7 天里 96.6% 的 10 分钟窗口零 IAM 变更事件
  ⇒ 作废本轮不会把闸门变成永远红（变更集中在本地 10–15 与 21–23 点，即刚部署完那段，
  正好是最该等一会儿再跑的时候）。
  **它是一道「窗口两端一致性」闸门，不要叫它原子。** 这个措辞被收窄过一次：两点比较
  只能证明 T1 与 T3 相等，证明不了中间没变过。**三个已接受的盲区**：① **改了又改回来**
  （T1 有授权 → T2 前撤掉，A 的模拟看到的是撤掉后的状态 → T3 恢复成与 T1 逐字相同）；
  ② 枚举后新建、复查前删除（两端都不存在，整轮不知道它来过）；③ **单次枚举自己也不是
  一个时刻**——`GetAccountAuthorizationDetails` 分页实测跨约 90 秒，翻页期间的变化不可见。
...
- **不完整的观测不会变成一个权威的绿**：`--no-asset-scan` 只能用于纯
  ...
  而 `--from-dump` 要求它**是布尔真**才放行——判据必须是 `is not True`：
  `"false"` 这个字符串是 truthy，写成 `if not bundle.get(...)` 就会放它过去。
- **观测的形状是一条递归默认拒绝的合同**（`BUNDLE_SHAPE`），不是"顶层几个分节在不在"。
  这一条被修过两轮，每轮都是同一个洞往下一层：先是缺 `coverage` / `iam_write` 整节
  （比较器拿到 None 就整层跳过），再是分节在、**内层键**没了（`coverage = {}` 让 774 项
  判不出的全变成"改善"；`resource_policies` 缺 `sites` 或**某个站点**的 shape 截断成
  `{}` 都让逐站点检查一个不做，且**零 note**）。
  **规律：单向比较器的层必须由合同兜住下限。** 双向比较的那些（B 的语句、boundary、
  bucket policy、平台逐桶——消失也红）内层缺了会自己红出来；单向的（消失=改善、或只
  遍历本次观测）不会。所以合同做成递归 + **规格外的键一律拒**，加新分节时不必再去
  判断它是哪个方向。
  **不声称**：合同只封"键缺失 / 类型不对"。`"sites": {}`（键在、一个成员都没有）与
  "这个账号真的没有站点"在数据上无法区分，闸门不猜——那条归部署验收（§9 的 3e）。
...
**守卫的反向验证也要 fail-closed**（`site-builder/scripts/metamorphic_trust_boundary.py`）

全绿只说明"当前实现让这些断言通过"，不说明那些断言**真的能红**——这道闸门被外部复审
六轮，每轮都至少有一条"加了守卫但守卫不生效"。所以每条关键守卫都有一次可复跑的变形：
改一行 → 跑指定用例 → 必须红 → 还原。

判据不能只看"pytest 退出码非 0"。实测三类**根本没跑到测试**的情形都会被认证成"红了"：
`-k` 因为用例改名选不到任何用例（rc=5）、测试文件语法错误（rc=2）、以及最隐蔽的
——**闸门脚本**语法错误（rc=1、末行 `1 failed`，与守卫真的红了逐字同形，因为测试在
函数体内 `exec_module` 那个脚本）。而变形本身是机械字符串替换，缩进错一格就是这个情形。
另外还必须证明变形**之前**那批用例是绿的，否则"本来就红、变形后继续红"也会被当成证据。

所以每条变形过四关：① 变形前 `rc == 0` 且至少选中一条用例；② 变形后文件仍可编译/导入；
③ 变形后 `rc == 1`；④ 末行有实际的 `N failed`。任何一关不过都报成**变形本身坏了**，
不算"守卫红了"。
