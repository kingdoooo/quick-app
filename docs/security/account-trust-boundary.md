...
- **枚举与逐个模拟之间有一个约 10 分钟的窗口（TOCTOU）。** 先
  ...
  **2026-08-27 起，模拟结束后会再枚举一次并逐个比对**（`uid` = RoleId/UserId、
  boundary、带来源的全部语句），三类里任一类出现即**作废本轮**——不出结论、不写基线
  ...
  **三条实测事实**（生产 CloudTrail 查得）：① 同名重建在本账号是常态——2026-08-27
  ...
  正好是最该等一会儿再跑的时候）。
  **这道复查只覆盖 principal 层。** 函数清单、alias、resource policy、Edge 代码与
  asset 各自在自己的时刻测得，跨层不保证原子——别把它读成"整轮都原子了"。
